                                    ...
                                    <div class="card h-100 border rounded overflow-hidden">
                                        <div class="position-relative">
                                            ...
                                            <a href="{{ route('product.show', $produto->id) }}">
                                                <div style="height: 140px; background-color: #f8f9fa; display: flex; align-items: center; justify-content: center;">
                                                    <img src="{{ asset($imagem) }}" alt="{{ $produto->nome }}" style="max-height: 140px; max-width: 100%; object-fit: contain;">
                                                </div>
                                            </a>
                                            
                                            <!-- Badge de desconto -->
                                            <div class="position-absolute top-0 end-0 bg-danger text-white px-2 py-1" style="font-size: 0.75rem;">-{{ $desconto }}%</div>
                                            
                                            <!-- Ícone para remover dos favoritos -->
                                            <form action="{{ route('favorites.remove') }}" method="POST" class="position-absolute top-0 start-0 m-1">
                                                @csrf
                                            ...
                                            <!-- Ícone para ver produto -->
                                            <a href="{{ route('product.show', $produto->id) }}" class="position-absolute bottom-0 end-0 m-1 btn btn-sm btn-primary rounded-circle" style="width: 30px; height: 30px; padding: 0; line-height: 30px;">
                                                <i class="fa-solid fa-eye"></i>
                                            </a>
                                        </div>
                                        
                                        <div class="card-body p-2 d-flex flex-column">
                                            <a href="{{ route('product.show', $produto->id) }}" class="text-decoration-none text-dark">
                                                ...
                                                <div class="d-flex align-items-center mb-1">
                                                    <span class="text-primary fw-bold">R$ {{ number_format($produto->preco, 2, ',', '.') }}</span>
